...
- AI role: the live path uses OpenAI to interpret ambiguous request evidence and help draft copy.
- Safeguards: deterministic rules keep missing accessibility, age guidance, weather, and acknowledgement details visible after the AI step.
...
| See what a coordinator must review | `examples/workshop_bulletin_run/organizer_digest.md` | `request_evidence.json`, `request_interpretation.json`, `normalized_request.json` |
| Inspect the AI interpretation and drafting check | `examples/evaluations/openai_draft_eval/eval_summary.md` | `eval_report.json`, `evals/draft_eval_cases.json` |
...
- [`examples/workshop_bulletin_run/request_interpretation.json`](examples/workshop_bulletin_run/request_interpretation.json) records the interpretation layer that classifies those gaps before normalization.
...
The top-fold packet in `examples/workshop_bulletin_run/` is a checked-in captured OpenAI sample created from the synthetic fixture. It is review evidence for the live interpretation and drafting path, not the artifact that `npm run demo` regenerates in a no-secrets checkout.
...
- `npm run eval` scores the interpretation-and-drafting workflow against the checked-in rubric in `evals/draft_eval_cases.json`.
...
- [`examples/evaluations/openai_draft_eval/eval_summary.md`](examples/evaluations/openai_draft_eval/eval_summary.md) for the checked-in OpenAI interpretation and drafting eval summary.
...
The workflow can call the real OpenAI interpretation and drafting steps on the same checked-in fixture:
...
- `evals/` contains the dataset and rubric inputs for the interpretation and drafting eval.
